...
import IndicatorAddDialog from '../components/IndicatorAddDialog.vue'
...
// 类型选项
const typeOptions = [
  { label: '定性指标', value: '定性指标' },
  { label: '定量指标', value: '定量指标' }
]
...
// 新增提交
const handleAddSubmit = (payload: { name: string; status: string }) => {
  ...
  tableData.value.unshift(newIndicator)
  ElMessage.success('新增成功')
}

// 查看
const handleView = (row: Indicator) => {
  console.log('查看', row)
  ElMessage.info(`查看指标: ${row.name}`)
}

// 启用/禁用
const handleToggleStatus = (row: Indicator) => {
...
// 删除
const handleDelete = (row: Indicator) => {
  ElMessageBox.confirm(
    '确定要删除该指标吗？删除后不可恢复',
    '警告',
    {
...
<template>
  <div class="indicator-management">
    <div class="content-card">
      ...
      >
        <el-table-column prop="name" label="指标名称" min-width="590" />
