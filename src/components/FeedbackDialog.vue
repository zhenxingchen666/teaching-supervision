<script setup lang="ts">
import { ref, watch } from 'vue'
import { Close } from '@element-plus/icons-vue'

const props = defineProps<{
  visible: boolean
}>()

const emit = defineEmits<{
  (e: 'update:visible', val: boolean): void
  (e: 'submit', payload: { type: string; description: string; anonymous: boolean }): void
}>()

const feedbackType = ref('')
const description = ref('')
const isAnonymous = ref(true)

const feedbackTypeOptions = [
  { value: 'feature', label: '功能建议' },
  { value: 'bug', label: '问题反馈' },
  { value: 'experience', label: '体验问题' },
  { value: 'other', label: '其他' },
]

watch(
  () => props.visible,
  (v) => {
    if (v) {
      feedbackType.value = ''
      description.value = ''
      isAnonymous.value = true
    }
  },
)

function close() {
  emit('update:visible', false)
}

function onSubmit() {
  emit('submit', {
    type: feedbackType.value,
    description: description.value,
    anonymous: isAnonymous.value,
  })
  close()
}
</script>

<template>
  <Teleport to=".design-canvas" defer>
    <div v-if="visible" class="feedback-mask" @click.self="close">
      <div class="feedback-dialog">
        <!-- banner: 520×300, only top strip is visible above the white card -->
        <div class="dialog-banner">
          <h3 class="dialog-title">意见反馈</h3>
          <button class="close-btn" @click="close">
            <el-icon><Close /></el-icon>
          </button>
        </div>

        <!-- white content card overlaps the banner: 480×446 at (20, 66) -->
        <div class="dialog-content">
          <!-- 反馈类型 -->
          <div class="form-field type-field">
            <label class="field-label">反馈类型</label>
            <el-select
              v-model="feedbackType"
              placeholder="请选择"
              class="type-select"
              popper-class="feedback-type-popper"
            >
              <el-option
                v-for="opt in feedbackTypeOptions"
                :key="opt.value"
                :label="opt.label"
                :value="opt.value"
              />
            </el-select>
          </div>

          <!-- 描述: 顶部位图提示 + 分割线 + 输入区 -->
          <div class="form-field desc-field">
            <label class="field-label">描述</label>
            <div class="desc-textarea">
              <div class="desc-hint">
                <img
                  src="/feedback-desc-hint.png"
                  alt=""
                  class="desc-hint-img"
                />
              </div>
              <div class="desc-divider"></div>
              <el-input
                v-model="description"
                type="textarea"
                :rows="6"
                placeholder="请输入"
                resize="none"
                class="desc-input"
              />
            </div>
          </div>

          <!-- 匿名 -->
          <div class="anon-block">
            <label class="anon-title">匿名</label>
            <el-radio-group v-model="isAnonymous" class="anon-radio-group">
              <el-radio :value="true">是</el-radio>
              <el-radio :value="false">否</el-radio>
            </el-radio-group>
          </div>
        </div>

        <!-- footer buttons -->
        <div class="dialog-footer">
          <el-button class="btn-cancel" @click="close">取消</el-button>
          <el-button type="primary" class="btn-submit" @click="onSubmit">提交</el-button>
        </div>
      </div>
    </div>
  </Teleport>
</template>

<style scoped>
.feedback-mask {
  position: absolute;
  inset: 0;
  background: rgba(0, 0, 0, 0.6);
  display: flex;
  align-items: center;
  justify-content: center;
  z-index: 2000;
  animation: fade-in 0.2s ease;
}

@keyframes fade-in {
  from { opacity: 0; }
  to { opacity: 1; }
}

.feedback-dialog {
  position: relative;
  width: 520px;
  height: 580px;
  background: #e7f2ff;
  border-radius: 18px;
  box-shadow: 0 20px 60px rgba(0, 0, 0, 0.25);
  overflow: hidden;
  animation: pop-in 0.25s cubic-bezier(0.16, 1, 0.3, 1);
}

@keyframes pop-in {
  from { transform: scale(0.92); opacity: 0; }
  to { transform: scale(1); opacity: 1; }
}

/* banner: 520×300 */
.dialog-banner {
  position: absolute;
  inset: 0 0 auto 0;
  height: 300px;
  background: url('/feedback-dialog-bg.png') center / cover no-repeat;
  border-radius: 18px 18px 0 0;
  z-index: 1;
}

.dialog-title {
  position: absolute;
  top: 21px;
  left: 0;
  right: 0;
  margin: 0;
  text-align: center;
  font-size: 18px;
  font-weight: 600;
  color: #ffffff;
  line-height: 25px;
  letter-spacing: 0.5px;
  text-shadow: 0 1px 4px rgba(0, 30, 80, 0.25);
}

.close-btn {
  position: absolute;
  top: 20px;
  right: 20px;
  width: 26px;
  height: 26px;
  border-radius: 50%;
  border: none;
  background: rgba(255, 255, 255, 0.8);
  color: #191c1e;
  display: flex;
  align-items: center;
  justify-content: center;
  cursor: pointer;
  box-shadow: 0 2px 6px rgba(0, 0, 0, 0.08);
  transition: background 0.15s, transform 0.15s;
  padding: 0;
}

.close-btn:hover {
  background: #ffffff;
  transform: scale(1.05);
}

.close-btn .el-icon {
  font-size: 14px;
}

/* white content card: 480×446 at (20, 66) */
.dialog-content {
  position: absolute;
  top: 66px;
  left: 20px;
  width: 480px;
  height: 446px;
  background: #ffffff;
  border-radius: 12px;
  box-shadow: 0 4px 16px rgba(20, 56, 140, 0.08);
  padding: 20px;
  z-index: 2;
  display: flex;
  flex-direction: column;
  gap: 22px;
}

.form-field {
  display: flex;
  flex-direction: column;
  gap: 8px;
}

.field-label {
  padding-left: 8px;
  font-size: 14px;
  color: #262626;
  line-height: 20px;
}

/* 反馈类型 select 180×36 */
.type-select {
  width: 180px;
}

.type-select :deep(.el-select__wrapper) {
  min-height: 36px;
  height: 36px;
  border-radius: 6px;
  background: #ffffff;
  box-shadow: 0 0 0 1px #e7e7e9 inset;
  padding: 0 12px;
}

.type-select :deep(.el-select__wrapper:hover) {
  box-shadow: 0 0 0 1px #1947ff inset;
}

.type-select :deep(.el-select__wrapper.is-focused) {
  box-shadow: 0 0 0 1px #1947ff inset !important;
}

.type-select :deep(.el-select__placeholder) {
  font-size: 14px;
  color: #bfbfbf;
}

.type-select :deep(.el-select__selected-item) {
  font-size: 14px;
  color: #262626;
}

/* 描述 textarea container 440×210 */
.desc-field {
  flex-shrink: 0;
}

.desc-textarea {
  width: 440px;
  height: 210px;
  border: 1px solid #e7e7e9;
  border-radius: 6px;
  background: #ffffff;
  display: flex;
  flex-direction: column;
  overflow: hidden;
  transition: border-color 0.15s;
}

.desc-textarea:focus-within {
  border-color: #1947ff;
  box-shadow: 0 0 0 1px rgba(25, 71, 255, 0.12);
}

.desc-hint {
  height: 32px;
  padding: 5px 10px;
  display: flex;
  align-items: center;
  flex-shrink: 0;
}

.desc-hint-img {
  width: 417px;
  height: 22px;
  object-fit: contain;
  display: block;
  pointer-events: none;
  user-select: none;
}

.desc-divider {
  height: 1px;
  background: #e7e7e9;
  flex-shrink: 0;
}

.desc-input {
  flex: 1;
  min-height: 0;
}

.desc-input :deep(.el-textarea) {
  height: 100%;
}

.desc-input :deep(.el-textarea__inner) {
  height: 100%;
  min-height: 100%;
  border: none;
  border-radius: 0;
  box-shadow: none;
  resize: none;
  padding: 8px 12px;
  font-size: 14px;
  color: #262626;
  background: transparent;
  font-family: inherit;
  line-height: 22px;
}

.desc-input :deep(.el-textarea__inner::placeholder) {
  color: #bfbfbf;
}

.desc-input :deep(.el-textarea__inner:focus) {
  border: none;
  box-shadow: none;
}

/* 匿名: label 在上, 是/否 单选在下 */
.anon-block {
  display: flex;
  flex-direction: column;
  gap: 8px;
}

.anon-title {
  padding-left: 8px;
  font-size: 14px;
  color: #262626;
  line-height: 20px;
}

.anon-radio-group {
  display: inline-flex;
  align-items: center;
  gap: 16px;
  padding-left: 8px;
}

.anon-radio-group :deep(.el-radio) {
  margin-right: 0;
  height: 20px;
  --el-radio-input-border-color-hover: #1947ff;
  --el-radio-text-color: #000000;
  --el-radio-input-border-color: #e5e5e5;
}

.anon-radio-group :deep(.el-radio__label) {
  font-size: 14px;
  padding-left: 6px;
  color: #000000;
}

.anon-radio-group :deep(.el-radio__inner) {
  width: 16px;
  height: 16px;
  border: 1px solid #e5e5e5;
}

.anon-radio-group :deep(.el-radio__input.is-checked .el-radio__inner) {
  background: #1947ff;
  border-color: #1947ff;
}

.anon-radio-group :deep(.el-radio__input.is-checked .el-radio__inner::after) {
  background-color: #ffffff;
  width: 6px;
  height: 6px;
}

.anon-radio-group :deep(.el-radio__input.is-checked + .el-radio__label) {
  color: #000000;
}

/* footer */
.dialog-footer {
  position: absolute;
  bottom: 16px;
  left: 0;
  right: 0;
  display: flex;
  justify-content: center;
  gap: 12px;
  z-index: 3;
}

.dialog-footer .el-button {
  width: 60px;
  height: 36px;
  border-radius: 8px;
  font-size: 14px;
  margin: 0;
  padding: 0;
}

.dialog-footer .btn-cancel {
  background: #ffffff;
  border: 1px solid #d9d9d9;
  color: #262626;
}

.dialog-footer .btn-cancel:hover {
  background: #ffffff;
  border-color: #1947ff;
  color: #1947ff;
}

.dialog-footer .btn-submit {
  background: #1947ff;
  border: 1px solid #1947ff;
  color: #ffffff;
  box-shadow: 0 4px 10px rgba(25, 71, 255, 0.25);
}

.dialog-footer .btn-submit:hover {
  background: #3a63ff;
  border-color: #3a63ff;
}
</style>

<style>
.feedback-type-popper .el-select-dropdown__item.is-selected {
  color: #1947ff;
  font-weight: 500;
}

.feedback-type-popper .el-select-dropdown__item:hover {
  background-color: #f5f8ff;
}
</style>
